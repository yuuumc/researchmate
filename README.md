...
本项目在开发过程中使用了 AI 辅助工具（基于 DeepSeek 大语言模型及 AI 编程助手），具体使用范围如下：

- **代码编写**：前端组件、后端 API 函数、核心引擎逻辑（Router 编排、GraphRAG 检索、三层记忆、情绪安全边界等）的部分代码由 AI 辅助生成，经人工审核、测试和调整后采用
- **提示词设计**：Agent 提示词模板（`prompts/` 目录下各 `.md` 文件）由 AI 辅助设计和优化
- **技术文档**：项目技术文档部分内容由 AI 辅助撰写，经人工校对

所有 AI 生成内容均经过人工审核与功能测试验证，确保代码质量、功能正确性和安全性。项目核心架构设计、产品需求定义和技术方案决策均由人工完成。
