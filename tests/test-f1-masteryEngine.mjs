...
const ks0 = { mastery: 80, confidence: 0.5, lastStudied: _now0, attempts: 5, correctRate: 0.6, errorTypes: {} }
const decayed0 = applyDecay(ks0, _now0)
check('衰减 0 天：mastery 不变', decayed0.mastery === 80)

// 30 天衰减：mastery 下降
const oldDate = new Date(Date.now() - 30 * 86400000).toISOString()
const ks30 = { mastery: 80, confidence: 0.5, lastStudied: oldDate, attempts: 5, correctRate: 0.6, errorTypes: {} }
const decayed30 = applyDecay(ks30, new Date().toISOString())
check('衰减 30 天：mastery 下降', decayed30.mastery < 80)
check('衰减 30 天：mastery 约 80*exp(-1.5) 约 17.8', decayed30.mastery > 15 && decayed30.mastery < 21)

// decayAll
const ksMap = {
  'topic_a': { mastery: 80, confidence: 0.5, lastStudied: oldDate, attempts: 5, correctRate: 0.6, errorTypes: {} },
  'topic_b': { mastery: 60, confidence: 0.3, lastStudied: new Date().toISOString(), attempts: 2, correctRate: 0.5, errorTypes: {} },
}
const { map: decayedMap, changed } = decayAll(ksMap, new Date().toISOString())
check('decayAll：7天知识点 mastery 下降', decayedMap['topic_a'].mastery < 80)
...
const baseKS = { mastery: 50, confidence: 0.5, lastStudied: new Date().toISOString(), attempts: 5, correctRate: 0.6, errorTypes: {} }
...
check('答对：推导 mastery 增幅 > 客观题', (afterDerivation.mastery - 50) > (afterChoice.mastery - 50))
...
const ksLowAttempts = { mastery: 50, confidence: 0.5, lastStudied: new Date().toISOString(), attempts: 1, correctRate: 0.5, errorTypes: {} }
const ksHighAttempts = { mastery: 50, confidence: 0.98, lastStudied: new Date().toISOString(), attempts: 50, correctRate: 0.8, errorTypes: {} }
...
check('attempts=1 单次增幅 > attempts=50', (afterLowAttempts.mastery - 50) > (afterHighAttempts.mastery - 50))
check('attempts=50 单次影响 < 1', Math.abs(afterHighAttempts.mastery - 50) < 1)
...
const ksBefore = { mastery: 60, confidence: 0.8, lastStudied: new Date(Date.now() - 86400000).toISOString(), attempts: 4, correctRate: 0.75, errorTypes: { choice: 1 } }
...
check('答错后 mastery 下降', ksAfterWrong.mastery < 60)
...
check('答对后 mastery 上升', ksAfterCorrect.mastery > 60)
...
const ks7 = { mastery: 80, confidence: 0.8, lastStudied: new Date(Date.now() - 7 * 86400000).toISOString(), attempts: 10, correctRate: 0.8, errorTypes: {} }
const decayed7 = applyDecay({ ...ks7 }, new Date().toISOString())

check('7天衰减后 mastery < 80', decayed7.mastery < 80)
check('7天衰减约 80*exp(-0.35) 约 56.4', decayed7.mastery > 50 && decayed7.mastery < 62)
check('衰减后 lastStudied 不变', decayed7.lastStudied === ks7.lastStudied)

// 快照测试
const snapKS = applySnapshot({ mastery: 80, attempts: 10, lastStudied: 'old' }, { mastery: 40, timestamp: new Date().toISOString() })
check('快照设定 mastery=40', snapKS.mastery === 40)
check('快照保留 attempts=10', snapKS.attempts === 10)
check('快照更新 lastStudied', snapKS.lastStudied !== 'old')

// mastery <-> stars 转换
check('mastery 40 -> 2星(weak)', masteryToStars(40) === 2)
check('mastery 60 -> 3星(dev)', masteryToStars(60) === 3)
check('mastery 80 -> 4星(strength)', masteryToStars(80) === 4)
check('mastery 100 -> 5星', masteryToStars(100) === 5)
check('stars 3 -> mastery 60', starsToMastery(3) === 60)
...
check('答错后 mastery 下降', mockStore.knowledge_state['高等数学'].mastery < 100)
...
profileBus.emit(EVT.MASTERY_SNAPSHOT, {
  items: [
    { topic: '概率论', mastery: 80, source: 'diagnosis' },
    { topic: '数理统计', mastery: 40, source: 'diagnosis' },
  ],
  timestamp: new Date().toISOString(),
})
check('快照后 概率论 mastery=80', mockStore.knowledge_state['概率论'].mastery === 80)
check('快照后 数理统计 mastery=40', mockStore.knowledge_state['数理统计'].mastery === 40)
