...
- `{{suggested_score}}` — 系统基于学生自评星级（1-5星）计算的建议分数（0-100），是评分的核心锚点
...
2. **诊断分数**（`score`）— 以 `suggested_score` 为核心锚点，结合知识结构分析微调：
   - 如果输入中包含 `suggested_score`，最终 `score` 必须在 `suggested_score ± 10` 范围内
   - 学生自评星级高（4-5星多）→ 高分（75-95）；中等（3星为主）→ 中分（45-65）；低（1-2星多）→ 低分（20-40）
   - 仅当 `suggested_score` 缺失时，才按"已掌握知识点数 / 考纲总数 × 100"估算
...
1. **严禁 LLM 生成数字字段**：`{{score}}` / `{{accuracy_rate}}` / `{{rank}}` 由前端渲染，LLM 不得在 reason / JSON 中输出任何正确率 / 排名数字。**例外**：`score` 字段是诊断分数，由 LLM 结合 `suggested_score` 锚点估算，这是唯一允许 LLM 生成的数字。**分数锚定规则**：如果输入 JSON 中包含 `suggested_score`，最终 `score` 必须在 `suggested_score ± 10` 范围内，不得偏离；自评 5 星为主的学生分数不得低于 70，自评 1-2 星为主的学生分数不得高于 45
